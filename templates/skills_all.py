...
def main():
    ...
    data = pd.read_csv("vacancies.csv", usecols=['key_skills', 'published_at'])

    # Удаление записей с отсутствующими навыками
    data.dropna(subset=['key_skills'], inplace=True)
...
def creating_plot(df, year):
    ...
    plt.tight_layout()
    plt.savefig(f"skills/graphs/graph_top_20_by_{year}.png")
    return


def creating_html_tables(year, table):
    # Создание HTML-таблицы и сохранение в файл
    f1 = open(f"skills/tables/top_20_by_{year}.html", "w", encoding='utf-8')
    f1.write(table)
    f1.close()
    return
