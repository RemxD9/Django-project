...
def main():
    data = pd.read_csv("vacancies.csv", usecols=['salary_from', 'salary_to', 'salary_currency', 'area_name', 'published_at'])
    ...
    data = data.dropna(subset=['salary_currency']).reset_index(drop=True)
